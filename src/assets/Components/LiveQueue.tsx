...
const LiveQueue: React.FC = () => {
  ...
  const loadQueueData = () => {
    try {
      ...
      const currentAppointment = localStorage.getItem(
  ...
  const updateQueueStatus = (queueData: QueuePatient[]): QueuePatient[] => {
    ...
    updatedQueue.forEach((patient, index) => {
      if (patient.status === "in-session") {
        ...
        patient.waitTime = Math.max(
          0,
          (patient.position - 2) * 5 - Math.floor(Math.random() * 3)
        );
      }
    });
    ...
    setQueue((prevQueue) => {
      ...
      if (currentUserIndex > 0) {
        const patientsAhead = Math.max(
          0,
          currentUserIndex -
            updatedQueue.filter((p) => p.status === "completed").length -
            1
        );
  ...
  const handleGetNotification = () => {
    Swal.fire({
      ...
      html: `You will be notified when it's your turn.<br><small>Approximately ${formatTime(
        estimatedWait
      )} remaining</small>`,
      icon: "success",
  ...
  const calculateProgress = () => {
    ...
    return Math.min(
      100,
      Math.round((completedCount / Math.max(1, queue.length - 1)) * 100)
    );
  };
  ...
  const getCurrentStatus = () => {
    ...
    switch (currentUser.status) {
      case "your-turn":
        return `Your turn in ${Math.max(0, currentUser.position - 2)} position${
          currentUser.position - 2 !== 1 ? "s" : ""
        }`;
      case "waiting":
        return `${formatTime(estimatedWait)} remaining`;
      case "in-session":
                    ...
                    <div>
                      ...
                      {patient.status === "your-turn" && (
                        ...
                        >
                          ...
                          Your turn in {Math.max(0, patient.position - 2)}{" "}
                          position{patient.position - 2 !== 1 ? "s" : ""}
                        </span>
                      )}
              ...
              <div className="space-y-1 text-sm text-gray-600">
                <p style={{ fontFamily: "'Inter', sans-serif" }}>
                  ...
                  <span className="text-blue-600">
                    {calculateEstimatedConsultationTime()}
                  </span>
                </p>
                <p style={{ fontFamily: "'Inter', sans-serif" }}>
